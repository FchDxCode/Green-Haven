...
class ImageFloraInline(admin.TabularInline):
    model = ImageFlora
    extra = 1
...
class ImageFloraAdmin(ModelAdmin):
    list_display = ('flora', 'image', 'created_at', 'updated_at')
    list_filter = ('flora',)

    def save_model(self, request, obj, form, change):
        super().save_model(request, obj, form, change)
        # Track image addition/update
        event_name = 'flora_image_updated' if change else 'flora_image_added'
